...
int conditional(int x, int y, int z) {
  int non_zero = !x<<31>>31;
  return (~non_zero & y ) | (non_zero & z);
}
/* 
...
unsigned float_neg(unsigned uf) {
  ...
  unsigned topMost=0xFF000000;
  unsigned temp = uf<<1;
  if((topMost & temp) ==  topMost)
  {
       if(temp != topMost)
	 return uf;
  }
...
int float_f2i(unsigned uf) {
  int smallest_exp = 8388608;
  int e = 157 - ((uf >> 23) & 0xFF);
  int fb = 8388607;
  int b = (smallest_exp | (uf & fb)) << 0x7;
  int result;
  if (e > 31)
      return 0;
  else if (e < 0)
